...
def cleanup_all_task_artifacts() -> None:
    """Best-effort cleanup for task-owned temp files (called at shutdown)."""
...
def _ingest_csv(task_id: str, temp_path: str) -> None:
    ...
    try:
        ...
        row_count = conn.execute("SELECT COUNT(*) FROM movies_staging").fetchone()[0]
        if row_count == 0:
        ...
        db.set_task(task_id, progress=100, status="completed")
...
async def query_movies(
    ...
    response.headers["X-Query-Time"] = f"{result['query_time']:.3f}"

    # Fast-path query completed within threshold; no need to keep task state.
    _cleanup_task_artifacts(db.delete_task(task_id))
    return [Movie(**row) for row in result["movies"]]


# ---------------------------------------------------------------------------
# Async Query (SSE for long-running queries)
# ---------------------------------------------------------------------------


@router.post("/movies/query", status_code=202)
async def query_movies_async(
